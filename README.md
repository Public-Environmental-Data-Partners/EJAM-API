...
- `sitenumber` - default `0` = aggregate **multisite report**; a positive integer reports on that one site. When only one site is supplied, N > 1 labels the header "Site N" (that site's row in the original analysis) instead of "Site 1", same as GET.
